...
public class UpdateRequestHandler extends SimpleChannelInboundHandler<UpdateRequestPacket>{
	...
	void UserUpdate(ChannelHandlerContext ctx,DBoperation db, UpdateResponsePacket updateResponsePacket)
	{
		...
		Map<String,HashMap<Time,Integer>> map = new HashMap<String,HashMap<Time,Integer>>();// 教室，日期，状态
		for(int i = 0 ;i<infoList.size(); i++)
		{
			...
			{
				map.get(room).put(new Time(infoList.get(i).getStarttime(),infoList.get(i).getEndtime()),0);
			}
			else
			{
				map.put(room, new HashMap<Time,Integer>());
				map.get(room).put(new Time(infoList.get(i).getStarttime(),infoList.get(i).getEndtime()), 0);
			}
		}
		for(int i = 0; i<userInfoList.size();i++)
		{
			...
			map.get(room).put(new Time(userInfoList.get(i).getStarttime(),userInfoList.get(i).getEndtime()),1);
		}
